...
export async function main() {
  ...
  graph.canvas.addEventListener(
    "mousedown",
    (ev: MouseEvent) => graph.onMouseDown(ev),
  );
  graph.canvas.addEventListener(
    "mousemove",
    (ev: MouseEvent) => {
  ...
  graph.canvas.addEventListener("mouseup", (ev: MouseEvent) => graph.onMouseUp(ev));
...
class Graph {
  ...
  constructor(nodes: GraphNode[], edges: Matrix) {
  ...
  update(dt: number) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      if (this.nodes[i].selected) {
        this.nodes[i].f = this.nodes[i].f.sub(
          this.nodes[i].p.sub(this.mousePos).mul(5),
        );
      }
      for (let j = 0; j < this.edges.n; j++) {
...
class GraphNode {
  ...
  constructor(value: number) {
    ...
    this.p = new Vec2(
      Math.round(Math.random() * 300) + (960 / 2) - 150,
      Math.round(Math.random() * 300) + (540 / 2) - 150,
    );
    this.f = new Vec2(0, 0);
    this.r = 20;
    this.selected = false;
